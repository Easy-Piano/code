...
public class KeyboardAdapterWithInput extends KeyAdapter {

    private Map<Integer, Integer> keyMap;
    protected Piano piano;
    private Vector[] startTimes = new Vector[Constants.NUM_OCTAVES*Constants.NUM_KEYS_PER_OCTAVE];
    private Vector[] endTimes= new Vector[Constants.NUM_OCTAVES*Constants.NUM_KEYS_PER_OCTAVE];
    ...
    public KeyboardAdapterWithInput(Piano piano) {
        this.piano = piano;
        initKeyMap();
        changeKeyMap(12);
    }
    ...
    public void pianoKeyReleased(int keyCode) {
        ...
        public synchronized void keyPressed(KeyEvent e) {
            ...
            if (isNote(keyCode)) {
                startTimes[getNote(keyCode)].add(System.currentTimeMillis() - startAnalyserTime);
                System.out.println("note " + getNote(keyCode) + " time " + startTimes[getNote(keyCode)].get(startTimes[getNote(keyCode)].size() - 1));
            }
        }
        ...
        public synchronized void keyReleased(KeyEvent e) {
            ...
            if (isNote(keyCode)) {
                endTimes[getNote(keyCode)].add(System.currentTimeMillis()-startAnalyserTime);
                System.out.println("note " + getNote(keyCode) +" time " + endTimes[getNote(keyCode)].get(endTimes[getNote(keyCode)].size()-1));
        }
    }

    public void setStartAnalyserTime(long startAnalyserTime) {
        this.startAnalyserTime = startAnalyserTime;
    }
}
